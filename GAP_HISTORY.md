...
### G234 — HF parity tolerance must scale with logit magnitude, not be a flat 1e-4

**Found:** Implementation phase GPU audit
**Symptom:** The CPU HARD GATE used `max_diff < 1e-4` and passed at `(B=1, T=4)` where logit magnitudes are small (≲ 30). On GPU at realistic shapes (`B=4, T=128`), logits grow to magnitude ~100 and the max absolute diff between our forward and HF's GPT-2 grows to ~1.6e-4 — just over the flat 1e-4 bound. HF GPT-2 is bit-deterministic (running it twice yields max-diff exactly 0.0), so the 1.6e-4 is the gap between OUR forward kernel selection and HF's. Relative to the logit magnitude, that's 1.6e-6 — ~1 ULP at fp32. The flat 1e-4 bound was implicitly only valid for the small-input regime.
**Root cause:** Floating-point matmul is non-associative; different kernel selection (SDPA backend / CUDNN heuristics) produces bit-different but mathematically-equivalent results. The accumulated drift across N layers scales with logit magnitude, which in turn scales with sequence length and batch size (more accumulated softmax sums in attention).
**Fix:** New GPU parity test uses `tolerance = max(1e-4, 2e-6 * |logits|_max)`. At small inputs the absolute bound dominates (1e-4); at larger inputs the magnitude-scaled bound takes over (~2e-4 at logit_max=100). Both maintain ~1e-6 relative precision. The CPU HARD GATE still uses the original flat 1e-4 (it's at small enough shapes that logits don't exceed ~30).
**Test:** `tests/parity/test_hf_logit_parity_gpu.py::test_logit_parity_at_realistic_size` swept across `(B, T) ∈ {(1, 4), (4, 32), (2, 64), (1, 128), (4, 128)}` on GPU; all pass.
**Affects:** `tests/parity/test_hf_logit_parity_gpu.py`. Documentation: future HF-parity tests at larger scales should use the scaled tolerance form, not a flat absolute bound.
...
### G235 — `eval.needle_in_haystack` still used the broken sliding-window NMM-reprocess pattern after Phase 7

**Found:** Post-Phase-7 audit
**Symptom:** Phase 7 fixed the architectural NMM-reprocess bug in `generate.py` (each decoded token re-fed the trailing `block_size` window through the NMM, compounding state updates ~`block_size`× per sampled token). The same broken pattern remained in `eval.py:needle_in_haystack`, which both (a) chunked the prompt through `model.forward()` to warm up NMM state, AND THEN (b) inside the decode loop took `window = ids[:, -block_size:]` and called `model(window, nmm_states, None)` per token — re-feeding the trailing window AND continuing to thread the already-updated `nmm_states` forward. The trailing `block_size` prompt tokens were thus passed through the NMM twice; each decoded token re-fed the entire window a third+ time. The smoke test `test_needle_in_haystack_runs_to_completion` only asserted `isinstance(result, bool)` and would not catch the bug.
**Root cause:** The Phase 7 architectural fix in `generate.py` was not propagated to the other inference-path caller. There was no regression guard locking the cached path in, so the divergence between the two decode pipelines went unnoticed.
**Fix:** Ported `eval.needle_in_haystack` to use `model.prepare_decode` + `model.forward_step` end-to-end, exactly mirroring `generate.py`. For long prompts (> `block_size`), use the same chunked-warm-up + `prepare_decode(tail, initial_nmm_states=...)` pattern with `max_new=1` at the boundary.
**Test:** Added `tests/integration/test_needle_smoke.py::test_needle_in_haystack_uses_cached_decode_path` — call counts `model.forward`, `model.prepare_decode`, and `model.forward_step` and asserts the harness uses ONLY the cached pipeline (`forward()=0`, `prepare_decode()=1`, `forward_step()>=1`). Reintroducing the old pattern would fail loudly via the `forward()` count.
**Affects:** `eval.py:needle_in_haystack`; `PLAN.md` §5.3 updated to call out the cached path; `RUNBOOK.md` "Long-context generation drift" §4 expanded to mention the needle harness was on the same path.

### G236 — `generate.py` off-by-one in `max_new` cap silently dropped one legitimate token

**Found:** Post-Phase-7 audit
**Symptom:** The cap was `max_new = min(max_new_tokens, block_size - prompt_len)`. Trace the decode loop: with `max_new=K`, `forward_step` is called K-1 times at positions `prompt_len, prompt_len+1, ..., prompt_len + K - 2`. The constraint is `prompt_len + K - 2 < block_size`, i.e. `K <= block_size - prompt_len + 1` — ONE more than the cap allowed. At `prompt_len == block_size` the user got an empty string back even though `cache["last_logits"]` was valid for one more sample without ever needing `forward_step`. For shorter prompts the user lost exactly one token. The runtime guard inside the loop (`cache["position"] < block_size`) prevented an actual OOB on the wpe table if the cap had been too LOOSE, so this never crashed — it just silently truncated.
**Root cause:** Conflating "number of forward_step calls needed" (K-1) with "number of decoded tokens" (K). The first decoded token comes from the warm-up's `cache["last_logits"]` and never hits `forward_step`.
**Fix:** Changed the cap to `min(max_new_tokens, block_size - prompt_len + 1)`. The long-prompt branch was already correct (`max_new = 1` at the boundary, which under the corrected formula evaluates to `block_size - block_size + 1 = 1`).
**Test:** `tests/unit/test_generate.py::test_generate_at_prompt_len_equals_block_size_returns_one_token` (asserts exactly 1 token decoded when `prompt_len == block_size`); `::test_generate_short_prompt_respects_new_cap` (asserts the cap permits `block_size - prompt_len + 1` tokens without OOB). The prompt-length-tooling helper `_prompt_with_encoded_length` builds a string whose `tok.encode(...)` length is exact, dodging the BPE round-trip noise that broke the first attempt at this test.
**Affects:** `generate.py` (the `else` branch's cap formula and the docstring `block_size - prompt_len` reference).

### G237 — Long-prompt cached-decode behavior parity test was smoke-only

**Found:** Post-Phase-7 audit
**Symptom:** `tests/behavior/test_cached_generate_parity.py::test_cached_decode_long_prompt_uses_full_context` ran the long-prompt path on two prompts with DIFFERENT prefixes but only asserted `isinstance(out_a, str)` and `isinstance(out_b, str)`. It even disclaimed in a comment: "The first decoded token may or may not differ depending on the model's sensitivity to the prefix; ... Here, just assert no crash and outputs are strings (smoke)." This meant the long-prompt path of the Phase 7 fix (chunked-warm-up + `prepare_decode(tail, initial_nmm_states=...)`) had NO behavior parity test — only call-counting and "doesn't crash". A regression where the prefix didn't actually thread through the NMM (e.g., `initial_nmm_states=None` accidentally) would not have failed any test.
**Root cause:** The author (me) backed off the strong invariant because at untrained random init the argmax token can coincide across different prefixes. The correct strong invariant — that `cache["last_logits"]` MUST differ — was overlooked.
...
### G249 — Chunked-warm-up + tail-prepare_decode pattern duplicated in three files

**Found:** Post-Phase-7 audit (fourth pass)
**Symptom:** The same chunked-warm-up + `prepare_decode(tail, initial_nmm_states=...)` pipeline appeared verbatim in three places: `generate.py:70-78`, `eval.py:99-105` (needle_in_haystack), and `tests/behavior/test_cached_generate_parity.py:115-123` (the parity test mirrored the production logic). Future risk: a bug fix in one file would not propagate to the others — exactly the failure mode that produced G235 (Phase 7's NMM-reprocess fix landed in `generate.py` but missed `eval.needle_in_haystack` for an entire pass cycle).
**Root cause:** Phase 7.4 wrote the long-prompt branch inline in `generate.py`; the subsequent G235 port to `needle_in_haystack` and the G237 parity test rewrite both copy-pasted the loop rather than extracting a helper. No reusable API existed for "prepare a decode cache from any prompt length".
**Fix:** Added a new `TitansMAGGPT2.prepare_decode_chunked(prompt_idx)` method that encapsulates the pattern. For prompts <= block_size it returns `prepare_decode(prompt_idx)` directly; for longer prompts it runs the chunked-warm-up loop through `forward()` then `prepare_decode(tail, initial_nmm_states=...)`. Same eval-mode contract as `prepare_decode` (G243) — the guard fires upfront so the warm-up loop's forward calls don't run their dropout-different path before the inner `prepare_decode` would reject. Refactored all three call sites to use the new method.
**Test:** `tests/integration/test_decode_parity.py::test_prepare_decode_chunked_short_prompt_matches_prepare_decode` (equivalence at short prompts), `::test_prepare_decode_chunked_long_prompt_threads_nmm_state_across_prefix` (prefix-sensitivity at long prompts via logit-level invariant — two prefixes yield different last_logits), `::test_prepare_decode_chunked_rejects_train_mode` (G243-parallel eval-mode guard).
**Affects:** `model/titans_gpt2.py` (new method); `generate.py` (refactored short+long branches → single helper call + per-branch max_new cap); `eval.py:needle_in_haystack` (same refactor); `tests/behavior/test_cached_generate_parity.py` (replaced inline construction with helper).

### G250 — Dead `ln_nmm` skip branch in `_apply_gpt2_init`

**Found:** Post-Phase-7 audit (fourth pass)
**Symptom:** `_apply_gpt2_init` had `if "ln_nmm" in name: continue` as an explicit skip. But the subsequent re-init logic only matches `nn.Linear` and `nn.Embedding` via `isinstance` — `LayerNorm` is neither, so it's already skipped implicitly. The branch was a no-op: removing it produces identical behavior. The comment ("LayerNorm default is already what we want") made it look like a safeguard, but the actual safeguard was the `isinstance` filter; a reader could spend time understanding the explicit branch only to learn it does nothing. Same logic applied to `ln_1`, `ln_2`, `ln_f` — none had explicit skips, all were correctly left at defaults.
**Root cause:** Defensive overspecification from an earlier pass when the init logic structure was less settled.
**Fix:** Removed the explicit `if "ln_nmm" in name: continue` branch. Added a one-line comment in its place documenting that the `isinstance(nn.Linear)/isinstance(nn.Embedding)` filter handles LayerNorm-skipping implicitly for ALL LayerNorms in the model (`ln_1`, `ln_2`, `ln_f`, `ln_nmm` all get the LayerNorm default of weight=1, bias=0).
**Test:** No new test — existing `test_full_model::test_apply_gpt2_init_*` continue to pass and assert the correct init values. (The `ln_nmm` independence test `test_block::test_ln_nmm_appears_in_state_dict_independent_of_ln_1` covers that the LayerNorm exists separately.)
**Affects:** `model/titans_gpt2.py:_apply_gpt2_init`.

### G251 — `needle_in_haystack` lacked a batched / aggregated harness

**Found:** Post-Phase-7 audit (fourth pass)
**Symptom:** The single-pair `needle_in_haystack(model, haystack, secret, insert_fraction)` function returns one bool — useful for smoke-testing but not for actually measuring recall. TEST_PLAN.md §10 specifies the production-relevant test as "Top-1 token includes '42' for ≥80% of positions" — an aggregated recall metric across many positions. No function in the codebase produced that metric; callers would have had to write the sweep loop themselves and aggregate, which is exactly the kind of duplication G249 was about.
**Root cause:** The single-pair function was written first; the aggregated sweep ("real harness") was acknowledged as future work in the docstring ("for a real harness this would batch many (position, secret) pairs and report accuracy vs context length") but never implemented.
**Fix:** Added `eval.needle_in_haystack_sweep(model, tokenizer, device, haystack, ...)`. Accepts either explicit `insert_fractions` + `secrets` lists or generates `n_positions` × `n_secrets` defaults (evenly spaced positions in (0, 1); random 5-char alphanumeric secrets seeded from `seed`). Returns a dict with `recall`, `n_pairs`, `n_matched`, `per_position` (dict[fraction, recall]), `per_secret` (dict[secret, recall]), and full `details` per pair. The matches-the-TEST_PLAN-§10-spec "recall" key is the headline metric. Untrained models produce near-zero recall (intended for trained-checkpoint evaluation).
**Test:** `tests/integration/test_needle_smoke.py` — six new tests covering: (1) default-grid structure (n_positions=9 × n_secrets=5 = 45 pairs), (2) per-position / per-secret aggregation correctness (recall matches mean of relevant details), (3) explicit overrides ignore default counts, (4) seed determinism (same seed -> same default secrets), (5) different seeds -> different secrets, (6) degenerate 1×1 grid runs one pair.
**Affects:** `eval.py` (new `needle_in_haystack_sweep` + `_random_secret` helper); `tests/integration/test_needle_smoke.py` (six new tests).

### G252 — Test coverage backfill against TEST_PLAN.md spec (T1–T10)

**Found:** Audit pass — compared TEST_PLAN.md §4–§13 spec'd test files vs files actually present in `tests/`.
**Symptom:** 10 gaps where the spec called for a test that didn't exist (or whose claimed defender was named in §14 but absent on disk). Most severely: the §14 regression matrix pointed G216 at `test_weight_loading::test_hf_model_name_from_n_embd`, but `test_weight_loading.py` did not exist — the matrix's "every gap has a defending test" promise was broken. Smaller items: no save/load numerical-equivalence resume test; no `doc_boundaries=all_true` reset invariant test; the overfit-batch test asserted only "trending down" instead of the spec'd `loss < 0.1`; no NaN-injection test for `run_training`'s accumulation-block path (separate from `train_step`'s NaN-skip); no multi-block-stack gradient-flow test; no DDP order-of-operations structural tests; no perf-tier smoke tests; no fuzzed invalid-config test; no positive whole-file `encode_corpus([f.read()])` test.
**Root cause:** Phase 7 and the subsequent audit batches focused on the code paths most likely to regress in the moment; the spec-driven coverage gaps accumulated across phases without anyone walking the spec front-to-back.
**Fix:** Added or strengthened 10 tests covering T1–T10:
  - **T1** — new `tests/unit/test_weight_loading.py` (10 tests): `_HF_GPT2_NAMES` table correctness, factory-to-HF-name routing, Conv1D transpose correctness for c_attn/c_proj/c_fc, LayerNorm + embedding copy without transpose, NMM params untouched by `load_pretrained`, n_layer/n_head mismatch raises. Uses a `_FakeHFModel` + monkey-patched `from_pretrained` so tests run without network. Defends G216.
  - **T2** — added 3 tests to `tests/integration/test_train_loop.py`: numerical resume parity (uninterrupted N+M-step run == N-step → save → load → M-step within 1e-5), optimizer-momentum-loaded behavioral check, resume-after-NaN-skip.
  - **T3** — added 3 tests to `tests/unit/test_full_model.py`: doc_boundaries=all_true isolates positions from earlier-input changes (with kernel_size=1), no-reset path DOES propagate changes (counterpart), state differs between reset and no-reset paths.
  - **T4** — added `test_overfit_batch_drives_loss_near_zero` to `tests/unit/test_train_step.py`: 200 steps, larger model (n_embd=32), higher LR; asserts final loss < 0.1 per TEST_PLAN §10 spec.
  - **T5** — added 2 tests to `tests/unit/test_train_step.py`: `run_training`'s accumulation-block NaN-skip leaves params unchanged; NaN-skip recovery — subsequent non-NaN cycles step the optimizer normally.
  - **T6** — added `test_multi_block_stack_forward_and_gradient_flow_to_every_param` to `tests/unit/test_full_model.py`: 3-block model, every named param has non-None grad after backward.
  - **T7** — added 5 AST-level tests to `tests/unit/test_train_main_structure.py`: init_process_group before DDP wrap, model.to(device) before DDP wrap, DDP wrap before build_optimizer, per-rank seed AFTER model construction (G204), init/destroy pair exists.
  - **T8** — new `tests/performance/test_perf_smoke.py` (4 tests, marked `perf`): train_step tokens/sec floor, no tensor-leak across steps, generate per-token latency doesn't grow super-linearly with output length, generate completes within wall-time bound.
  - **T9** — added 40 parametrized fuzz tests to `tests/unit/test_config.py`: 5 invalid-config kinds × 8 random seeds each; assert ValueError with informative message.
  - **T10** — added `test_encode_corpus_whole_file_pattern_produces_one_document` to `tests/unit/test_tokenizer.py`: positive case for the G210-recommended `encode_corpus([f.read()])` pattern.
