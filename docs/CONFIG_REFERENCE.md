...
| `nmm_layer_indices` | `Optional[list[int]]` | `None` | When set, NMM is wired only on the listed blocks; others become `PlainGPT2Block` (attention + MLP only, no NMM, no MAG gate; in `persistent_prefix_mode="model_wide"` they still see the model-level persistent prefix and apply a block-structured mask). Linear reduction of NMM-related compute and memory in the number of NMM layers. Paper applies NMM at every block; this is a deliberate departure for VRAM. Validation rejects out-of-range / duplicate / non-int indices. **Decode path is supported**: plain blocks contribute a KV cache only; the NMM state slot at plain-block positions is `None`. |
...
| Peak VRAM | **~8.5 GiB** (7 GiB headroom) |
| Throughput | **~920 tok/s** |
| Effective batch (`--grad-accum 16`) | 16 |
| Optimizer step wall-clock | ~18 s |
| 1k optimizer steps | ~5 h |
| 50k optimizer steps | ~10 days |

With `--nmm-use-gram-ns5`: the Gram-iteration variant replaces stock
NS5's 2T rectangular matmuls with 2 rectangular matmuls + T n×n
Gram-matrix iterations. End-to-end gain depends on what fraction of
your run is NMM (~78% of CUDA time at our recipe). See the "Faster
polar decomposition" sections below for the gram-NS5-vs-CANS choice.
...
The Gram-iteration variant (Tri Dao et al., POLAR_EXPRESS coefficients
+ reset at iter 2) reformulates NS5 so that the inner loop operates on
the small `n × n` Gram matrix (`R = X X^T`) instead of full rectangular
matmuls — 2 rectangular matmuls + T iterations on the small Gram
matrix, vs stock NS5's 2T rectangular matmuls. The "reset" at iter 2
re-orthogonalizes the intermediate result, which is why POLAR_EXPRESS
converges in 5 effective iterations.

**Implementation:** Pure PyTorch, in `model/nmm.py` (function
`gram_newton_schulz`). No external dependency. The earlier dependency
on Tri Dao's `gram-newton-schulz` package was dropped because:

* The library's `X /= X.norm(...) + eps` is an in-place divide that
  mutates a tensor saved for the norm's backward. Under
  `torch.compile`'s AOT autograd this trips the tensor-version check
  and raises `BackendCompilerFailed`. Our local copy uses an
  out-of-place divide.
* The library wraps `__call__` in `torch.compile(mode="reduce-
  overhead")` by default. CUDA-graph memory pools then pre-allocate
  per-shape buffers; with the NMM's many distinct call shapes this
  blew past 16 GiB VRAM at our recipe scale.
* The kernel-backed path (`quack-kernels` + CuTeDSL) requires
  Hopper/Blackwell + CUDA 12.9+ and is slower than cuBLAS at our
  batch=1 NMM shapes anyway (per `scripts/benchmark_ns5.py`).

**Quality note:** orthogonalisation error |sv−1| ≈ 0.12-0.15 on random
Gaussian inputs — comparable to stock NS5 with Muon coefficients. For
tighter orthogonalisation, use `--nmm-use-cans` (3-step minimax-tuned
coefficients).
...
`(3.4445, -4.7750, 2.0315)` — which Muon/Jordan et al. derived for an
idealised fp32 fixed point reached in 5 iterations — with coefficients
that are minimax-optimal over the *actual* post-Frobenius-normalisation
singular value range of our gradients.

**How the coefficients were derived:** We sampled singular values from
1000 random matrices at NMM weight shapes (768×3072 and 3072×768),
F-norm-normalised them, and took the p10..max range: σ ∈ [0.0228,
0.0542]. We then solved

    minimise   maxₛ |f³(σ) − 1|
    over       (a, b, c)
    where      f(x) = a·x + b·x³ + c·x⁵

via `scipy.optimize.differential_evolution`. The optimiser converged on
`(a, b, c) = (3.8641, -9.7196, 9.7101)`. See `scripts/benchmark_ns5.py`
for the exact procedure.

**Tradeoff vs gram-NS5:** CANS gives tighter orthogonalisation (~1.78
error vs gram-NS5's ~5.35), so it's the right pick when |sv−1| precision
matters. Gram-NS5's 5-iter polynomial has fewer per-call intermediates
to retain in the autograd graph, so it generally uses less activation
memory under `--compile-model`.

**Caveats:**

* The coefficients are tuned for the gpt2_small NMM shape regime
  (d=768, expansion=4, full-rank). At larger d, different expansion, or
  low-rank configurations, the sv range shifts. Re-derive coefficients
  via `scripts/benchmark_ns5.py` for those regimes.
* Three iterations is hard-coded — the coefficients are minimax-optimal
  for exactly 3 steps. Running 4 or 5 steps with these coefficients is
  *worse* than NS5-5.
* End-to-end step time improvement is smaller than the kernel speedup
  since non-NS work (forward/backward of the inner loss, optimizer step)
  is unchanged.
...
| Field | Type | Default | Notes |
|---|---|---|---|
| `retrieval_from_M_prev` | `bool` | `True` (paper Eq. 15) | Paper Eq. 15: `y_t = M(q_t)` where M is M_{t-1} (read-then-write). Default `True` = paper-strict. Flip to `False` for lucidrains write-then-read (retrieve from freshly-updated M_t). Applies to all NMM forward paths (`step_with_conv`, `_forward_chunk_sequential`, `_forward_chunk_blockwise`). |
...
| `persistent_prefix_mode` | `str` | `"model_wide"` (paper Eq. 19) | `"model_wide"` (default, paper-faithful): one `persistent_mem: [N_p, d]` Parameter at the `TitansMAGGPT2` level, prepended ONCE after wte+wpe, sliced before `ln_f`. Persistent positions propagate through every block and accumulate information from real tokens. `"per_block"` (legacy): each `TitansMAGBlock` owns its own `persistent_mem: [N_p, d]` (total persistent params: `N_p × d × n_layer`). At `nmm_n_persistent=0` both modes degenerate to a no-op prepend. |
