...
def test_no_sync_used_for_non_final_micro_batches():
    """G200: under DDP, all but the last micro-batch of a cycle must run
...
def test_partial_cycle_at_end_of_loader_skips_step_under_DDP():
    """G214/G222: under DDP, a partial accumulation cycle (StopIteration
    ...
    # chosen to fall at accum_i = K-1 (the G222 case).


def test_per_rank_seed_differs_after_model_construction():
    """G204: each rank gets seed + rank AFTER building the model so the
...
def test_init_process_group_before_DDP_wrap():
    """G201: init_process_group must be called before DistributedDataParallel
...
def test_destroy_process_group_runs_on_exception_path():
    """G225/G227: the entire training body is wrapped in try/finally with
