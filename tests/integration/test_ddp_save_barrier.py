...
def test_run_training_save_block_uses_rank0_guard_and_barrier():
    """Parse run_training; find the if-block that calls save_checkpoint;
    verify (a) it's guarded by `rank == 0` and (b) `dist.barrier()` is
    invoked in the same code region (under `is_distributed`)."""
    ...
    assert found_save, "run_training does not call save_checkpoint"
...
def test_save_path_is_inside_save_every_branch():
    ...
    src = _function_source(train.run_training)
    # Coarse check: "save_every" must appear and "save_checkpoint" must follow it.
    assert "save_every" in src
    assert "save_checkpoint" in src
    # `save_checkpoint` should come AFTER the `save_every` if-condition lexically.
